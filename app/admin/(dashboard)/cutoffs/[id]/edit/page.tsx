            ...
            .then(data => {
                if (data.cutoff) {
                    ...
                    setFormData({
                        ...
                        category: c.category || "UR",
                        postCode: c.postCode || "",
                        postName: c.postName || "",
                        expectedCutoff: String(c.expectedCutoff ?? ""),
                        safeScore: String(c.safeScore ?? ""),
                        minimumScore: String(c.minimumScore ?? ""),
                        previousYearCutoff: String(c.previousYearCutoff ?? ""),
                        previousYearVacancy: String(c.previousYearVacancy ?? ""),
                        currentYearVacancy: String(c.currentYearVacancy ?? ""),
                        confidenceLevel: c.confidenceLevel || "medium",
                        dataPoints: String(c.predictionBasis?.dataPoints ?? ""),
                        methodology: c.predictionBasis?.methodology || "",
                        factors: c.predictionBasis?.factors || [],
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 pb-10">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Edit Cutoff</h1>
                    <p className="text-gray-500 text-sm font-medium">Update cutoff prediction details.</p>
                </div>
            </div>
                ...
                <div className="card-base p-6 bg-white space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="md:col-span-2">
                            ...
                            <select disabled value={formData.examId}
                                className="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl bg-gray-50 text-sm font-medium text-gray-500 cursor-not-allowed">
                                <option value="">Select Exam...</option>
                                {exams.map(e => <option key={e.id} value={e.id}>{e.name} ({e.year})</option>)}
                            </select>
                            <p className="text-xs text-gray-400 mt-1">Exam cannot be changed after creation.</p>
                        </div>
                        <div>
