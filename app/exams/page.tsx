...
export default function ExamsPage() {
  ...
  const { getSubmission, isLoaded } = useSubmissionHistory();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            {["all", "active", "upcoming", "closed"].map((status) => (
              <button
                ...
                className={`px-6 py-2 font-bold border-2 border-black rounded-lg shadow-brutal-sm transition-all ${
                  filter === status
          ...
          {loading && (
            <div className="text-center py-20">
              <span className="material-symbols-outlined text-5xl text-primary animate-spin inline-block">
                autorenew
              </span>
          ...
          {!loading && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredExams.map((exam) => (
                <div
                  key={exam.id}
                  className={`relative ${examColors[exam.slug] || "bg-gray-50"} rounded-xl p-6 border-2 border-black shadow-brutal hover:-translate-y-1 transition-transform ${
                    exam.status === "closed" ? "opacity-70" : ""
                  }`}
                >
                  {/* Status Badge */}
                  <div
                    className={`absolute -top-3 -right-3 border-2 border-black text-xs font-black px-3 py-1 rounded shadow-brutal-sm transform ${
                      exam.status === "active"
                  ...
                  <div className="flex items-center text-xs font-bold text-gray-700 gap-4 pt-4 mb-4 border-t-2 border-dashed border-gray-300">
                    <span className="flex items-center gap-1">
                      ...
                      {exam.totalSubmissions
                        ? `${(exam.totalSubmissions / 1000).toFixed(0)}k+`
                        : "0"}{" "}
                      Data
                    </span>
                    <span className="flex items-center gap-1">
                  ...
                  {/* Action Button */}
                  {(() => {
                    const submission = getSubmission(exam.slug);
                    const hasSubmitted = !!submission;

                    if (exam.status === "active") {
                      if (hasSubmitted && isLoaded) {
                        return (
                          <Link
                            href={`/result/${submission.submissionId}`}
                            className="block w-full text-center py-2 px-4 bg-green-500 text-black font-black border-2 border-black rounded-lg shadow-brutal-sm hover:translate-y-0.5 hover:shadow-none transition-all uppercase text-sm flex items-center justify-center gap-2"
                          >
            ...
            <div className="relative inline-block">
              ...
              <div className="relative bg-brutal-yellow border-4 border-black rounded-xl p-8 shadow-brutal">
                ...
                <p className="text-base font-medium text-gray-700 mb-6">
                  We're constantly adding new exams. Check back soon or contact
                  us to request support for your exam.
                </p>
                <a
