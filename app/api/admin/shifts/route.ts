...
// GET - List all shifts with filters, sorting, and pagination
export async function GET(request: NextRequest) {
    try {
        ...
        const difficulty = searchParams.get("difficulty"); // label
        ...
        if (examId && examId !== "all") {
            conditions.push(eq(shifts.examId, parseInt(examId)));
        }
        if (dateFrom) {
            conditions.push(sql`${shifts.date} >= ${dateFrom}`);
        }
        if (dateTo) {
            conditions.push(sql`${shifts.date} <= ${dateTo}`);
        }
        if (timeSlot && timeSlot !== "all") {
            conditions.push(eq(shifts.timeSlot, timeSlot));
        }
        if (difficulty && difficulty !== "all") {
            conditions.push(eq(shifts.difficultyLabel, difficulty));
        }
        ...
        if (examIds.length > 0) {
            const examData = await db.select({ id: exams.id, name: exams.name }).from(exams).where(inArray(exams.id, examIds));
            examData.forEach((e) => {
